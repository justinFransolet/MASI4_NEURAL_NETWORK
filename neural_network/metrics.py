...
from .formula import matrix_confusion
...
def calculate_accuracy(y_true: list[float], y_pred_probs: list[float], threshold=0.5)-> float:
    ...
    """
    tp, tn, fp, fn = matrix_confusion(y_true, y_pred_probs, threshold)
    total = tp + tn + fp + fn

    return (tp + tn) / total if total > 0 else 0.0

def calculate_precision(y_true: list[float], y_pred_probs: list[float], threshold=0.5)-> float:
    """
...
def calculate_recall(y_true: list[float], y_pred_probs: list[float], threshold=0.5)-> float:
    """
...
def calculate_f1_score(y_true: list[float], y_pred_probs: list[float], threshold=0.5)-> float:
    """
    ...
    precision = calculate_precision(y_true,y_pred_probs, threshold)
    recall = calculate_recall(y_true,y_pred_probs, threshold)
